...
namespace BingoParser.UnitTests
{
   ...
   public static class UnitTests
   {
      ...
      [Test]
      public static void GetListOfFilesToRead() {
         ParseArguments.Parse(new string[0]);
         var localList = GetInputFileList(@"d:\(Sandbox)\DigheIdro\");

         var checkList = Directory.GetFiles(@"d:\(Sandbox)\DigheIdro\", "*.*");
         Assert.AreEqual(localList.Count, checkList.Length - 1);
      }
   }
}